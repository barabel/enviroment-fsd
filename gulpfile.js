...
const generateSvgEnum = require('./cli/generate-sprites/generate-sprites');
...
const paths = {
  ...
  src: {
    ...
    script: path.join(dirRoot, dirSrc, dirAssets, 'entry', '**/*.{ts,tsx}'),
...
function watch() {
  ...
  gulp.watch([paths.watch.js, `./${dirSrc}/views/**/**/*.{ts,tsx}`], scripts);
...
function server() {
  const proxyOptions = url.parse('http://localhost:3039/api');
